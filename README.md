# Projeto Cidade Inteligente – Educação

Este repositório reúne o desenvolvimento do meu trabalho sobre a Cidade Inteligente, com foco especial no eixo Educação, acompanhado da evolução ao longo das aulas.

# 🎯 Objetivo do Projeto

Explorar como soluções inteligentes podem transformar o setor educacional, utilizando tecnologia, dados e inovação para melhorar o aprendizado, a gestão escolar e o acesso à educação de qualidade.

# 📘 Tema Central
- Educação como Pilar de uma Cidade Inteligente

- Uso de tecnologias digitais para aprimorar processos educacionais.

- Integração de dados para tomada de decisões mais eficientes.

- Soluções inovadoras que aproximam alunos, professores e comunidade.

# 🛠️ Desenvolvimento ao Longo das Aulas
Este projeto foi construído de maneira incremental, conforme os conteúdos foram apresentados em sala

**Etapas do Projeto ciadade inteligente:**
- Modelo Conceitual
- Projeto Lógico
- Modelo Físico
- Dicionário de Dados


# 📈 Conclusão
- A educação é um dos pilares mais impactados positivamente pela tecnologia.

- Cidades inteligentes dependem diretamente de cidadãos bem informados e educados.

- A inovação pode reduzir desigualdades e melhorar significativamente o ensino.
